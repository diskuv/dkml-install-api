...
let create_forone_abi ~abi_selector ~all_component_names ~program_name
    ~program_version ~opam_context ~work_dir ~target_dir ~packager_setup_exe
    ~packager_uninstaller_exe =
  ...
  Populate_archive.populate_archive ~archive_dir ~packager_setup_exe
    ~packager_uninstaller_exe ~opam_context:(Fpath.v opam_context)
    ~all_component_names;
  (* Get Opam sources *)
...
let create_forall_abi (_log_config : Dkml_install_api.Log_config.t) program_name
    program_version work_dir target_dir opam_context abis packager_setup_exe
    packager_uninstaller_exe =
  (* Setup dune site *)
  ...
  List.iter
    (fun abi_selector ->
      create_forone_abi ~abi_selector ~all_component_names ~program_name
        ...
        ~packager_setup_exe:(Fpath.v packager_setup_exe)
        ~packager_uninstaller_exe:(Fpath.v packager_uninstaller_exe))
    abi_selectors
...
let setup_exe_t =
  ...
  Arg.(
    required
    & opt (some file) None
    & info [ "packager-setup-exe" ] ~docv:"EXE" ~doc)

let uninstaller_exe_t =
  let doc = "The uninstaller.exe generated by a (Console, etc.) packager" in
  Arg.(
    required
    & opt (some file) None
    & info [ "packager-uninstaller-exe" ] ~docv:"EXE" ~doc)
...
let create_installers () =
  let t =
    Term.(
      ...
      $ opam_context_t $ abis_t $ setup_exe_t $ uninstaller_exe_t)
  in
